...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  $app=$appResp.Content;" ^
  "  $appReady = $app.Contains('serverCommonTables') -and $app.Contains('tablePresetVersion') -and $app.Contains('tableTierLabels');" ^
  ...
  "  $ok = $health.ok -and (($tables.table_count -as [int]) -gt 0) -and ($invalidCount -eq 0) -and $html.Contains('targetDialog') -and $appReady -and $tierOrderOk;" ^
  "  [pscustomobject]@{ reachable=$true; ok=$ok; pid=$health.pid; tableCount=$tables.table_count; commonCount=$tables.common_tables.Count; invalidNameCount=$invalidCount; firstCommon=$firstCommon.name; firstTier=$firstCommon.frequency_tier; tierOrderOk=$tierOrderOk; htmlReady=$html.Contains('targetDialog'); appReady=$appReady; cacheControl=$appResp.Headers['Cache-Control'] } | ConvertTo-Json -Compress | Write-Host;" ^
