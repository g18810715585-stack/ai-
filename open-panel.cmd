...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if (-not ($html.Contains('targetDialog') -and $html.Contains('relationsBtn') -and $html.Contains('relationsTab') -and $html.Contains('diagnosticsTab') -and $html.Contains('teachBtn') -and $html.Contains('saveExperienceBtn') -and $html.Contains('experienceSummaryText') -and $html.Contains('activityPlanBtn') -and $html.Contains('planTab') -and $html.Contains('confirmationsTab') -and $html.Contains('internal-manifest') -and ($html.IndexOf('manifest-field') -lt 0))) { throw 'panel html is stale' }" ^
  "if (-not ($app.Contains('serverCommonTables') -and $app.Contains('tablePresetVersion') -and $app.Contains('tableTierLabels') -and $app.Contains('compactRelationshipMap') -and $app.Contains('compactDraftDiagnostics') -and $app.Contains('compactConfigPlan') -and $app.Contains('compactExperienceSummary') -and $app.Contains('setActionBusy'))) { throw 'app.js is stale' }" ^
  "exit 0"
exit /b %errorlevel%
