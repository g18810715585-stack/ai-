...
3. 在“配置表目录”里填写本地配置表根目录。这个输入会自动记住上次填写的路径。
4. 点击“扫描配置目录”，工具会按 Excel 的 sheet 页名称识别配置表，并排除说明页、空表等明显不是数据表的 sheet。
5. 点击“选择配置表”，在弹出的面板里勾选这次活动涉及的几张主要关联表。没有扫描结果时，可以先在“常用表列表”里粘贴 sheet 名并保存。
6. 点击“分析表格”，确认识别结果；再点击“生成草案”，让 AI 基于所选多张表分析字段和可能的主外键关系。
