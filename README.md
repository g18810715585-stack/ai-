...
7. 点击“分析表格”，确认识别结果和商品匹配结果；再点击“生成草案和预览”，让 AI 基于所选多张表分析字段和可能的主外键关系，并直接生成预览副本。
...
2. 新建配表项目，并在“飞书规划链接”里粘贴飞书文档/知识库表格/电子表格链接。
3. 在“配置表目录”里填你的本地配置表根目录，例如 `C:\TopHero\Meta\meta_local`。工具会递归扫描目录里的 `.xlsx/.xlsm`，但配置表身份以 sheet 页名称为准，不按 Excel 文件名判断。
4. 点击“扫描配置目录”，生成 `schema-draft.json` 和配置表扫描报告。
5. 点击“选择配置表”，勾选本次活动的主要关联表。
6. 按“分析关联关系 → 分析表格 → 识别活动模板 → 生成草案和预览”的顺序走完整流程。
7. 如果这次结果符合你的配表习惯，点击“记录习惯”；如果有问题，在“配表记录”里填写“本次问题修正”并总结为案例。
8. 用完后双击 `stop-panel.cmd`，关闭后台本地服务和端口。
...
1. 在“历史经验”页签里写一条自然语言规则，然后点“AI 整理经验”。工具会先把口语化经验整理成可编辑文本，并检查与历史经验是否冲突。历史经验只会沉淀个人规则和字段映射，不会自动创建活动模板。
2. 检查“整理结果”，必要时手动改几句，再点“保存经验”。经验会保存到本地 `.knowledge`，默认不进 Git。
3. 需要回看或维护经验时，仍在同一个弹窗内搜索、修改或删除历史经验。
4. “活动模板”页签可以维护兑换店、BP、积分任务、礼包、排行榜、掉落/收集等活动模板；只有这里手动新增或修改的内容才会进入活动模板库。
5. “字段字典”页签可以按表/字段维护含义、规划列名别名、是否可写、ID 新建/复用策略和风险说明。
6. 填好飞书规划链接、配置表目录，并选择这次活动的主要目标配置表后，点“识别活动模板”。工具会输出“配表计划”和“待确认字段”。
7. 确认计划后再点“分析关联关系”和“生成草案和预览”。草案只会生成待审核 Patch，低置信字段会进“待确认字段”；预览只写副本，不会硬写原表。
...
1. `生成草案和预览`：先生成待审核 Patch，再生成预览 Excel、diff、校验报告、rollback patch 和本次配表记录，不覆盖原表。
